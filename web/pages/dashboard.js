...
import { useRouter } from "next/router"; // <--- INI YANG HILANG TADI
import { database } from "../lib/fire";
...
export default function Dashboard() {
  ...
  // --- 0. CEK LOGIN (PROTEKSI) ---
  useEffect(() => {
    // Cek apakah user punya tiket masuk?
    const checkLogin = localStorage.getItem('isLoggedIn');
    
    if (!checkLogin) {
      // Kalau gak punya, tendang ke halaman login
      router.push('/'); 
    }
  }, []);
